class King < Piece
  ...
  def castle_move?(x, y)
    ...
    return false if (x - x_coordinate).abs != 2
    ...
    if y == y_coordinate
      ...
      if x > x_coordinate
        ...
        unless enemy_piece_threatens_castle_path?([6, 7], y) || friendly_piece_blocks_castle_path?([6, 7], y)
          # Query for King-side Rook
          king_side_rook = game.pieces.where(type: 'Rook', color: color, x_coordinate: 8).last

          # If the King-side Rook hasn't moved, move it now; otherwise return false
          king_side_rook ? king_side_rook.update_attribute(:x_coordinate, 6) : false
      ...
      # If castling attempting is Queen-side
        ...
        # rule: The king does not end up in check.
        # rule: There are no pieces between the King and the chosen Rook.
        unless enemy_piece_threatens_castle_path?([3, 4], y) || friendly_piece_blocks_castle_path?([2, 3, 4], y)
          # Query for Queen-side Rook
          queen_side_rook = game.pieces.where(type: 'Rook', color: color, x_coordinate: 1).last

          # If the Queen-side Rook hasn't moved, move it now; otherwise return false
          queen_side_rook ? queen_side_rook.update_attribute(:x_coordinate, 4) : false
  ...
  def enemy_piece_threatens_castle_path?(x_coordinates_of_path, y)
    ...
    enemy_pieces.each do |enemy_piece|
      x_coordinates_of_path.each do |x|
  ...
  def friendly_piece_blocks_castle_path?(x_coordinates_of_path, y)
    criteria = { game_id: game.id, color: color, x_coordinate: x_coordinates_of_path, y_coordinate: y, captured?: false }
